...
struct arp_hdr
{
    uint16_t hwtype;            // 2字节，链接层类型，以太网编号是0x0001
    uint16_t protype;           // 2字节，网络层协议，IPv4是0x0800
    uint8_t hwsize;             // 1字节，链路层地址长度，6字节
    uint8_t prosize;            // 1字节，网络层地址长度，4字节
    uint16_t opcode;            // 2字节，ARP消息类型。ARP request (1), ARP reply (2), RARP request (3) or RARP reply (4).
    unsigned char data[];       // ARP消息内容
} __attribute__((packed));

struct arp_ipv4                 // ARP消息IPv4的结构
{
    unsigned char smac[6];      // 源MAC地址
    uint32_t sip;               // 源IP地址
    unsigned char dmac[6];      // 目的MAC地址，请求时为广播地址，全1
    uint32_t dip;               // 目的IP地址，请求时为全0
} __attribute__((packed));
