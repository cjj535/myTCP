...
// 分配ARP条目的空间，RESOLVED表示占用，FREE表示已释放
...
void arp_rcv(struct sk_buff *skb)
{
    ...
    arphdr->hwtype = ntohs(arphdr->hwtype);             // 网络序转主机序
    arphdr->protype = ntohs(arphdr->protype);
    arphdr->opcode = ntohs(arphdr->opcode);
    arp_dbg("in", arphdr);

    if (arphdr->hwtype != ARP_ETHERNET) {               // 链路层不是以太网类型
        printf("ARP: Unsupported HW type\n");
        goto drop_pkt;
    }

    if (arphdr->protype != ARP_IPV4) {                  // 网络层不是IPv4
    ...
    merge = update_arp_translation_table(arphdr, arpdata);  // 更新ARP地址表
    ...
    if (!merge && insert_arp_translation_table(arphdr, arpdata) != 0) {     // 尝试插入新的条目，如果失败则说明没有足够空间
